...
const OrderCard = ({ order, currentLocale }: OrderCardProps) => {
  const t = useMemo(() => {
    ...
    <div className="bg-white flex flex-col" data-testid="order-card">
      ...
      <div className="flex items-center divide-x divide-gray-200 text-small-regular text-ui-fg-base">
        ...
        <span className="pl-2">{`${numberOfLines} ${
          numberOfLines > 1
            ? t("account.orders.items", "items")
            : t("account.orders.item", "item")
        }`}</span>
      </div>
      <div className="grid grid-cols-2 small:grid-cols-4 gap-4 my-4">
        ...
        {numberOfProducts > 4 && (
          <div className="w-full h-full flex flex-col items-center justify-center">
            <span className="text-small-regular text-ui-fg-base">+ {numberOfLines - 4}</span>
            <span className="text-small-regular text-ui-fg-base">{t("account.orders.more", "more")}</span>
          </div>
        )}
      </div>
      <div className="flex justify-end">
        <LocalizedClientLink href={`/account/orders/details/${order.id}`}>
          <Button data-testid="order-details-link" variant="secondary">
            {t("account.orders.seeDetails", "See details")}
          </Button>
        </LocalizedClientLink>
      </div>
    </div>
