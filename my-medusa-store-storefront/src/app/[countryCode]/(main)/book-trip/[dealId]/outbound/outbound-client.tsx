...
export default function OutboundClient({ deal, currentLocale, customerName, customerEmail }: OutboundClientProps) {
    ...
    async function load() {
      try {
        ...
        const mapped = (data.flights || []).map((f: any) => ({
          ...f,
          price_economy: f.current_price_economy ?? f.price_economy ?? 0,
          price_business: f.current_price_business ?? f.price_business ?? 0,
        }))
        setFlights(mapped)
      } catch (e) {
        console.error(e)
  ...
  const handleContinue = () => {
    ...
    const price = selectedClass === "economy"
      ? selectedFlight.price_economy ?? 0
      : selectedFlight.price_business ?? 0
  ...
  const selectedPrice = selectedFlight
    ? (selectedClass === "economy" ? selectedFlight.price_economy : selectedFlight.price_business) ?? 0
    : null
      ...
      <div className="container mx-auto px-4 max-w-3xl py-8">
        ...
        {loading ? (
          ...
          <div className="space-y-3 mb-8">
            {flights.map(flight => {
              const price = (selectedClass === "economy" ? flight.price_economy : flight.price_business) ?? 0
        ...
        {/* Continue */}
        {selectedFlight && selectedPrice !== null && (
